...
const WasteManagement = () => {
  ...
  const fetchEcoOrganisms = async () => {
    try {
      const response = await axios.get('/api/eco-organisms/active');
  ...
  const fetchDisposals = async () => {
    try {
      ...
      const response = await axios.get(`/api/waste-disposals?${params}`);
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get('/api/waste-disposals/stats/summary');
  ...
  const handleDeleteDisposal = async (id) => {
    if (!window.confirm('Êtes-vous sûr de vouloir supprimer cette sortie de déchets ?')) {
      return;
    }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Suivez et gérez les sorties de déchets vers les éco-organismes et déchetteries
      </Typography>
          ...
          <Grid item xs={12} sm={6} md={2}>
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                onChange={(e) => handleFilterChange('disposal_type', e.target.value)}
              >
            ...
            <TableBody>
              {disposals.map((disposal) => (
                <TableRow key={disposal.id}>
                  ...
                  <TableCell>
                    {new Date(disposal.disposal_date).toLocaleDateString('fr-FR')}
                  </TableCell>
                  <TableCell>
      ...
      >
        <DialogTitle>
          {editingDisposal ? 'Modifier la Sortie de Déchets' : 'Nouvelle Sortie de Déchets'}
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <FormControl fullWidth required>
                ...
                <Select
                  ...
                  onChange={(e) => setDisposalForm(prev => ({ 
                    ...
                    eco_organism_id: e.target.value !== 'eco_organism' ? '' : prev.eco_organism_id
                  }))}
                >
                  <MenuItem value="eco_organism">Éco-organisme</MenuItem>
                  <MenuItem value="landfill">Déchetterie</MenuItem>
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setDisposalForm(prev => ({ ...prev, subcategory_id: e.target.value }))}
                  disabled={!disposalForm.category_id}
                >
                  <MenuItem value="">Sélectionner une sous-catégorie</MenuItem>
                  {subcategories
                    .filter(sub => sub.category_id === parseInt(disposalForm.category_id))
                    .map((subcategory) => (
              ...
              <Grid item xs={12}>
                <FormControl fullWidth required>
                  ...
                  <Select
                    ...
                    onChange={(e) => setDisposalForm(prev => ({ ...prev, eco_organism_id: e.target.value }))}
                  >
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => setDisposalForm(prev => ({ ...prev, weight_kg: e.target.value }))}
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => setDisposalForm(prev => ({ ...prev, volume_m3: e.target.value }))}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => setDisposalForm(prev => ({ ...prev, transport_method: e.target.value }))}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => setDisposalForm(prev => ({ ...prev, transport_company: e.target.value }))}
              />
            </Grid>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) => setDisposalForm(prev => ({ ...prev, transport_cost: e.target.value }))}
              />
            </Grid>
        ...
        <DialogActions>
          <Button onClick={handleCloseDisposalDialog}>
            Annuler
          </Button>
      ...
      <Dialog open={showWeightKeypad} onClose={() => setShowWeightKeypad(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ p: 2 }}>
          <NumericKeypad
            ...
            onChange={(value) => setDisposalForm(prev => ({ ...prev, weight_kg: value }))}
